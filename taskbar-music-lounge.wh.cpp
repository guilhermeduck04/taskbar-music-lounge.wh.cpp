...
//
// - setting: TextColor
//   name: Cor do texto (HEX, ex: FFFFFF)
//   type: string
//   default: FFFFFF
//
// - setting: BgOpacity
//   name: Opacidade do fundo (0-255)
//   type: int
//   default: 64
//   min: 0
//   max: 255
...
static bool IsRectFullscreenOnMonitor(const RECT& rcWin, HMONITOR mon) {
    MONITORINFO mi{ sizeof(mi) };
    if (!GetMonitorInfo(mon, &mi)) return false;

    const RECT& rcMon = mi.rcMonitor;

    // tolerância pequena pra bordas/ajustes
    const int tol = 2;

    return (abs(rcWin.left   - rcMon.left)   <= tol) &&
           (abs(rcWin.top    - rcMon.top)    <= tol) &&
           (abs(rcWin.right  - rcMon.right)  <= tol) &&
           (abs(rcWin.bottom - rcMon.bottom) <= tol);
}

static bool IsFullscreenForegroundOnMonitor(HMONITOR mon, HWND selfHwnd) {
    HWND fg = GetForegroundWindow();
    if (!fg) return false;

    // ignora a própria janela e elementos do shell
    if (fg == selfHwnd) return false;
    ...
    // janela deve estar no mesmo monitor do taskbar/painel
    if (MonitorFromWindow(fg, MONITOR_DEFAULTTONULL) != mon) return false;

    RECT rc{};
    if (!GetWindowRect(fg, &rc)) return false;

    // algumas janelas "fullscreen" não são realmente fullscreen (ex: maximized com bordas),
    // então conferimos cobertura do monitor.
    return IsRectFullscreenOnMonitor(rc, mon);
}
    ...
    switch (msg) {
        ...
        case WM_TIMER:
            if (wParam == IDT_POLL_MEDIA) {
                ...
                if (hTaskbar) {
                    HMONITOR taskbarMon = MonitorFromWindow(hTaskbar, MONITOR_DEFAULTTONEAREST);

                    // Se tiver app em fullscreen nesse monitor: some junto com a taskbar
                    if (IsFullscreenForegroundOnMonitor(taskbarMon, hwnd) || !IsWindowVisible(hTaskbar)) {
                        if (IsWindowVisible(hwnd)) ShowWindow(hwnd, SW_HIDE);
                        return 0; // não reposiciona enquanto estiver fullscreen
                    } else {
                    ...
                    if (myRc.left != x || myRc.top != y ||
                        ...
                        // IMPORTANTE: NOTOPMOST aqui ajuda muito
                        SetWindowPos(hwnd, HWND_NOTOPMOST, x, y, g_Settings.width, g_Settings.height,
